...
Let us mention a conjecture related to this lemma, which states that there is equality for some $p$:
...
The result now follows by induction, we omit it since it is purely technical.
\end{proof}
...
\end{align}
The last inequality holds for all $\varepsilon > 0$, from which $2 ^ {1+d} < d^5$. This has integer solutions only for $d \leq 20$. Therefore there can be only finitely many better core dissections (for fixed $d$ and $\gamma$ there is single best dissection).
