...
\cosyalign{
	f:(x,y,z) \mapsto \big((x \bmod n), (y \bmod n), (z \bmod n)\big)
}%
...
\end{cor}%
\begin{proof}
The lower bound is \todo{Theorem}. For the upper bound, combine Lemma \ref{lem:gdist-n-leq-gdist-p}, Lemma \ref{lem:gdist-leq-tn} and Theorem \ref{thm:t-log-bound} to get
\cosyalign{
...
It would be desirable to construct a chain of better and better dissections that converge to the expected bound proposed in Chapter \todo{4}. However, the following lemmas show that using this method, this is not possible.
