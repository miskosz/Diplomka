...
Unless specified otherwise, in the rest of this chapter we use \emph{triangle} instead of \emph{equilateral triangle} for brevity.
...
Recall that $\Delta_n$ denotes a triangle of side~$n$.
...
\todo{@ Presunut do uvodu Chap4?}

\begin{note}
The key to establish the upper bound was the connection of latin bitrades to dissections of equilateral triangles, namely the fact that $\gdist(p) \leq t(p)$. However, it seems that the following conjecture might be true:
\end{note}%
...
\todo{This would allow us to find bound of $\gdist(n)$ by examining triangle dissections only.}
