...
\cosyalign{
 	\sigma_R(r,c,s) = (r',c,s) \mbox{ such that } r \ne r',\\
 	\sigma_C(r,c,s) = (r,c',s) \mbox{ such that } c \ne c', \\
 	\sigma_S(r,c,s) = (r,c,s') \mbox{ such that } s \ne s'.
}%
The definition of latin bitrade implies that these maps are involutions. They correspond to the edges of the graph -- on Figure \ref{fig:graph-bitrade},  $\sigma_R, \sigma_C, \sigma_S$ are represented by solid, dashed and dotted edges respectively.
...
The following results regarding the relation of a latin bitrade and its graph in the planar case are due to Cavenagh and Lisoněk \cite{CavenaghLisonek08}. We state them without a proof. Unlike us, they considered latin bitrades to be unordered pairs of partial latin squares. We call them \emph{unordered latin bitrades}.
